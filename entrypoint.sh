...
if [ -n "$PEERS" ]; then
  ...
  echo "{\"root_node_ips\": $ROOT_IPS, \"try_new_peers\": true, \"chain\": \"Mainnet\"}" > ~/override_gossip_config.json
...
# Start hl-visor
exec ~/hl-visor run-non-validator --write-fills --batch-by-block --replica-cmds-style recent-actions --serve-info
